	...
	void sort(address* addresses, int size) {
		for (int i = 0; i < size; i++) {
			for (int j = 0; j < size; j++) {
				if (addresses[j].get_output_address() < addresses[i].get_output_address())
				{
					int temp = addresses[i].get_output_address();
					addresses[i].get_output_address() = addresses[j].get_output_address();
					addresses[j].get_output_address() = temp;
				}
			}
		}
				for (int i = 0; i < size; i++)
